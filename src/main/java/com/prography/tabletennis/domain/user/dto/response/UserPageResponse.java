...
public class UserPageResponse {
	...
	@Builder
	public UserPageResponse(Integer totalElements, Integer totalPages, List<UserInfoResponse> userList) {
	...
	public static UserPageResponse from(Page<User> users) {
		List<UserInfoResponse> userList = users.getContent().stream().map(UserInfoResponse::from).toList();
		return UserPageResponse.builder().totalPages(users.getTotalPages())

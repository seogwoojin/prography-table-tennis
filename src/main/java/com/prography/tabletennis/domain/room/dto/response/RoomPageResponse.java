...
public class RoomPageResponse {
	...
	@Builder
	public RoomPageResponse(Integer totalElements, Integer totalPages, List<RoomInfoResponse> userList) {
	...
	public static RoomPageResponse from(Page<Room> rooms) {
		List<RoomInfoResponse> userList = rooms.getContent().stream().map(
			RoomInfoResponse::from).toList();
